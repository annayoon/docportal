...
- 언어 혼입(Qwen 특성): qwen2.5:14b가 한국어 답변에 일본어 가나·한자를 섞는
  문제 → 요약 프롬프트(summarizer.py)와 챗봇 앱 프롬프트 모두에 '한국어 전용,
  가나·한자 금지' 규칙 추가. 챗봇 프롬프트는 deploy/maxkb_fix_lang.py로 갱신.
  프롬프트로 크게 줄지만 100% 억제는 아님 — 재발 심하면 답변 모델 교체 고려.

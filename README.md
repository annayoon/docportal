...
- **형식 변환 다운로드** — 업로드 문서를 PDF·Markdown으로, 위키를 HTML/PDF/Word/
...
- **위키 문서 작성** — 마크다운으로 문서를 작성/편집. 편집할 때마다 버전이
  기록되고 이전 버전 열람 가능.
- **구조 잡기 (위키 목차 뼈대)** — 새 위키 문서 작성 시 문서 타입(튜토리얼·
  How-to·레퍼런스·릴리스 노트·공문/보고서 등 10종)과 주제·독자·목표를 입력하면,
  섹션별 "답해야 할 질문"과 분량 가이드가 주석으로 달린 목차 뼈대가 본문에
  자동으로 채워짐. 빈 화면에서 시작하지 않고 검증된 문서 구조에서 출발.
  ([Technical Writing Helper](https://github.com/annayoon/Technical-Writing-Helper)
  코어 사용, AI/외부 서비스 불필요)
- **부서/태그 분류** — 부서별 필터, 태그 검색.
- **미리보기 & AI 요약** — PDF·이미지는 브라우저에서 바로 보고, Word·PPT·Excel은
  PDF로 변환해 문서 모양 그대로 미리보기(해시 캐시 — 버전당 1회 변환,
  LibreOffice 필요). HWP 등 변환 불가 형식은 추출 텍스트 표시. 업로드하면 자동으로 요약 생성 —
